...
export function DistributedBoard() {
  ...
  return (
    <section className="panel mt-6 p-5 sm:p-6">
      ...
      <p className="num mt-3 font-display text-4xl tracking-tight">{preview ? usd(totalUsd) : "—"}</p>
      <p className="mt-2 max-w-md text-sm text-muted">
        {preview
          ? "Cumulative mark value sent to holders."
          : "Starts when the book lists and the keeper's first cycle clears."}
      </p>
      {preview ? (
